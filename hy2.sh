...
show_menu() {
    ...
    elif systemctl list-unit-files ss-ipv6.service &>/dev/null;
        then ss_status="${RED}已停止${ENDCOLOR}"
    fi
...
detect_network() {
    ...
    for svc in "${ipv4_svcs[@]}"; do
        IPV4_ADDR=$(curl -4 -s --connect-timeout 3 "$svc" 2>/dev/null | grep -Eo '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$' || true)
        [[ -n "$IPV4_ADDR" ]] && break
    done
...
check_port() {
    ...
    if $cmd | grep -q ":$port\s"; then
        error_echo "端口 $port/$protocol 已被占用"
        return 1
    fi
    return 0
}
...
hy2_install_dependencies() {
    ...
    declare -A pkg_map
    case "$OS_TYPE" in
        "ubuntu"|"debian") pkg_map=([curl]="curl" [wget]="wget" [jq]="jq" [openssl]="openssl" [nslookup]="dnsutils") ;;
        *) pkg_map=([curl]="curl" [wget]="wget" [jq]="jq" [openssl]="openssl" [nslookup]="bind-utils") ;;
    esac

    for cmd in "${!pkg_map[@]}"; do
        command -v "$cmd" &>/dev/null || pkgs_to_install+=("${pkg_map[$cmd]}")
    done
...
# 修复：恢复正确的 hy2_get_user_input 函数
hy2_get_user_input() {
    exec </dev/tty
    info_echo "开始配置 Hysteria2..."
    while true; do read -rp "请输入您的域名 (用于SNI): " DOMAIN; [[ -n "$DOMAIN" ]] && break; done
...
hy2_run_install() {
    ...
    if systemctl list-unit-files hysteria-server.service &>/dev/null; then
        warning_echo "检测到 Hysteria2 已安装，继续将覆盖。"
        read -rp "确定吗? (y/N): " confirm && [[ ! "$confirm" =~ ^[yY]$ ]] && return
        hy2_uninstall
    fi
...
ss_check_ipv6() {
    ...
    if [[ -z "$IPV6_ADDR" ]]; then
        error_echo "未能检测到公网 IPv6 地址！无法安装 Shadowsocks (IPv6-Only)。"
        return 1
    fi
...
ss_install_dependencies() {
    ...
    local pkgs_to_install=()
    local deps=("shadowsocks-libev" "qrencode")
    for pkg in "${deps[@]}"; do
        case "$OS_TYPE" in
            "ubuntu"|"debian")
                ...
                ;;
            *)
                rpm -q "$pkg" &>/dev/null || pkgs_to_install+=("$pkg")
                ;;
        esac
    done
    if [[ ${#pkgs_to_install[@]} -gt 0 ]]; then
        ...
        case "$OS_TYPE" in
            "ubuntu"|"debian") apt-get update -qq && apt-get install -y "${pkgs_to_install[@]}" ;;
            *) command -v dnf &>/dev/null && dnf install -y epel-release && dnf install -y "${pkgs_to_install[@]}" || yum install -y epel-release && yum install -y "${pkgs_to_install[@]}" ;;
        esac || { error_echo "依赖安装失败"; return 1; }
    fi
}

# 补全：完成 ss_get_user_input 函数
ss_get_user_input() {
    exec </dev/tty
    ...
    while true; do
        ...
        check_port "$SS_PORT" "tcp" && check_port "$SS_PORT" "udp" && break
    done
...
{
    "server": "::", "server_port": ${SS_PORT}, "password": "${SS_PASSWORD}",
    "method": "${SS_METHOD}", "mode": "tcp_and_udp", "fast_open": true
}
EOF
}
...
ss_configure_firewall() {
    ...
    if command -v ufw &>/dev/null && ufw status | grep -q "Status: active"; then
        ufw allow "${SS_PORT}" comment "Shadowsocks" >/dev/null
    elif command -v firewall-cmd &>/dev/null && systemctl is-active --quiet firewalld; then
...
ss_run_install() {
    if systemctl list-unit-files ss-ipv6.service &>/dev/null; then
        warning_echo "检测到 Shadowsocks (IPv6) 已安装，继续将覆盖。"
        read -rp "确定吗? (y/N): " confirm && [[ ! "$confirm" =~ ^[yY]$ ]] && return
        ss_uninstall
    fi
    ...
    echo; info_echo "配置二维码:"
    qrencode -t UTF8 "$(grep "ss://" /root/ss_ipv6_info.txt)"
}
...
manage_single_service() {
    ...
    while true; do
        ...
        case $op_choice in
            ...
            4) clear; journalctl -u "$service_name" -n 100 --no-pager; read -rp "按回车继续..." ;;
...
show_config_info() {
    clear
    if [[ ! -f /root/hysteria2_info.txt && ! -f /root/ss_ipv6_info.txt ]]; then
        error_echo "未安装任何服务。"; read -rp "按回车返回..." ; return
    fi
    if [[ -f /root/hysteria2_info.txt ]]; then
        echo -e "${PURPLE}--- Hysteria2 配置 ---${ENDCOLOR}" && cat /root/hysteria2_info.txt && echo
    fi
    if [[ -f /root/ss_ipv6_info.txt ]]; then
        echo -e "${PURPLE}--- Shadowsocks (IPv6) 配置 ---${ENDCOLOR}" && cat /root/ss_ipv6_info.txt
        echo; info_echo "二维码:"; qrencode -t UTF8 "$(grep "ss://" /root/ss_ipv6_info.txt)" && echo
    fi
    read -rp "按回车返回主菜单..."
}
...
uninstall_services() {
    while true; do
        ...
        case $choice in
            ...
            3) warning_echo "将卸载所有服务及其依赖！"
               read -rp "确定吗? (y/N): " confirm && [[ "$confirm" =~ ^[yY]$ ]] && { hy2_uninstall; ss_uninstall; success_echo "清理完成"; } ;;
            0) return ;;
            *) error_echo "无效选择" ;;
        esac
        read -rp "按回车返回..."
...
main() {
    ...
    while true; do
        ...
        case $main_choice in
            1) hy2_run_install "self" ;;
            2) hy2_run_install "acme" ;;
            3) ss_run_install ;;
            4) manage_services ;;
            5) show_config_info ;;
        ...
        [[ "$main_choice" =~ ^[123]$ ]] && read -rp "按回车返回主菜单..."
    done
}
