...
install_cloudflared() {
    ...
    CLOUDFLARED_PATH=$(command -v cloudflared)
    if [[ -z "$CLOUDFLARED_PATH" ]]; then error_echo "Cloudflared 安装失败"; exit 1; fi
...
install_acme_and_cert() {
    ...
    if ! ~/.acme.sh/acme.sh --issue --dns dns_cf -d "$DOMAIN" --server letsencrypt --force --ecc; then
        error_echo "SSL 证书申请失败！请检查 acme.sh 日志。"; exit 1
    fi
    ...
    for ((i=1; i<=port_check_timeout; i++)); do
        if ss -ulnp | grep -q ":443.*hysteria"; then
            ...
            if [[ -n "$CLOUDFLARED_PATH" ]]; then
                ...
                info_echo "等待 Cloudflared 隧道连接 (8秒)..." && sleep 8
            fi
            return 0
        fi
        echo -n "." && sleep 1
