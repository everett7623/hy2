...
EOF
    ...
    echo -e "${GREEN}╠════════════════════════════════════════════════════════════════╣${ENDCOLOR}"
    echo -e "${GREEN}║  服务器地址: ${YELLOW}$(printf "%-47s" "$DOMAIN")${GREEN}║${ENDCOLOR}"
    ...
    echo -e "${GREEN}╠════════════════════════════════════════════════════════════════╣${ENDCOLOR}"
    echo -e "${GREEN}║  管理命令:   ${YELLOW}$(printf "%-47s" "hy2-manage [start|stop|restart|status|log|uninstall]")${GREEN}║${ENDCOLOR}"
    echo -e "${GREEN}╚════════════════════════════════════════════════════════════════╝${ENDCOLOR}"
    echo
...
case "$1" in
    start)
        ...
        ;;
    stop)
        info_echo "停止 Cloudflared 和 Hysteria2 服务..."
        systemctl stop cloudflared hysteria-server
        success_echo "服务停止完成"
        ;;
    restart)
        # [IMPROVED] 重启逻辑更稳健
        ...
        info_echo "正在重启 Hysteria2..."
        systemctl restart hysteria-server
        info_echo "等待 Hysteria2 稳定..."
        sleep 3
        if ! systemctl is-active --quiet hysteria-server; then
            error_echo "Hysteria2 服务重启失败，请用 'hy2-manage log' 查看日志。"
            exit 1
        fi
        info_echo "正在启动 Cloudflared..."
        systemctl start cloudflared
        sleep 2
        success_echo "服务重启成功。"
        show_status
    ...
    uninstall)
        ...
        if [[ "$confirm" != "y" && "$confirm" != "Y" ]]; then info_echo "取消卸载"; exit 0; fi
        ...
        warning_echo "Cloudflared 本体未卸载, 您可使用包管理器手动移除 (例如: apt purge cloudflared)"
        ;;
    *)
        echo "用法: hy2-manage [start|stop|restart|status|log|cflog|uninstall]"
        exit 1
        ;;
esac
EOF
...
# 脚本入口: 根据传入参数决定是执行安装还是调用管理脚本
if [[ $# -gt 0 ]]; then
    if [[ ! -f /usr/local/bin/hy2-manage ]]; then
        error_echo "管理脚本不存在，请先完整运行一次安装脚本。"
        exit 1
    fi
    check_root
    /usr/local/bin/hy2-manage "$@"
else
    main_install
fi
