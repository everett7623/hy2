...
warn_streaming_egress() {
    if [ "${WARP_ACTIVE:-0}" = "1" ] && [ -n "${DEFAULT_EGRESS_IPV4:-}" ] && [ -n "${PUBLIC_IP:-}" ] && \
        ...
        echo -e "${RED}        流媒体（如 Netflix）可能提示 proxy/VPN；SS 无法绑定出站网卡，请关闭 WARP。${PLAIN}"
    fi
...
# 是否存在默认 IPv4 路由。用于在公网 IP 探测站不可达时，
# 仍能确认本机具备 IPv4 出网能力。
...
detect_network() {
    ...
    NAT_MODE=0; IPV6_ONLY=0; HAS_IPV4=0; HAS_IPV6=0; PUBLIC_IP=""; PUBLIC_IPV6=""; DEFAULT_EGRESS_IPV4=""; WARP_ACTIVE=0; IPV4_UNVERIFIED=0
    local _ip _url
    ...
    if is_valid_ipv4 "$_ip"; then
        ...
        # 原生网卡有全局 IPv4 且存在默认 IPv4 路由即认定具备 IPv4；
    ...
    if [ "$WARP_ACTIVE" = "1" ]; then
        ...
        if [ -n "$PUBLIC_IP" ] && [ -n "$DEFAULT_EGRESS_IPV4" ] && [ "$DEFAULT_EGRESS_IPV4" != "$PUBLIC_IP" ]; then
            ...
            echo -e "  ${RED}注意: 默认出站与节点 IP 不一致，流媒体可能判 VPN；SS 无法绑定出站网卡，请关闭 WARP 或修正路由${PLAIN}"
        fi
    fi
...
# Shadowsocks-Rust 脚本写入 - 标准 BBR 优化
...
main_menu() {
    while true; do
        ...
        echo -e "${SKYBLUE}===============================================${PLAIN}"
        echo -e "${GREEN}  Shadowsocks-Rust Management Script v2.0.35${PLAIN}"
        echo -e "${SKYBLUE}===============================================${PLAIN}"
