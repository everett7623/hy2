...
- 修复 UFW 状态判断包含匹配 `active`，导致 `inactive` 被误判为已启用并触发端口放行失败的问题；状态与规则检查统一使用英文 locale 和精确匹配。
- sing-box 候选核心执行失败时保留退出码与原始错误，并针对 OOM、运行库/执行权限、CPU 指令集问题给出明确提示。
- 新增 Alpine 构建选择和 UFW 启用、停用、重复放行、失败路径的回归测试。
...
- 影响面：从 Windows 复制的密码常带尾随回车符（CR）。经改密码路径写入时不会被拒绝，CR 会进入 YAML 配置与分享链接（编码为 `%0D`），服务能正常启动，但客户端使用的密码与服务端不一致，连接失败且全程没有任何报错。
- 新增 `valid_hy2_password()` 单点定义，安装与改密码共用，规则取双引号 YAML、分享链接和 `awk` 替换文本三处危险字符的并集。改用 `printf` 而非 `echo` 传入，避免 `echo` 解释反斜杠转义。
- 已核对其余脚本：`ss.sh` 的 `valid_json_secret()`、`anytls.sh` 与 `proxy.sh` 的 `validate_password()` 均已在安装与修改路径共用，无同类问题；`vless.sh` 使用带校验的 UUID，不涉及。
- 审计结论（无需改动）：分享链接的 `uri_encode()` 采用严格白名单 `[a-zA-Z0-9.~_-]`，`@ : / ? #` 均会被百分号编码，密码不会破坏 URI 结构；`yaml_single_quote_escape()` 的单引号加倍处理正确。
- 测试：新增密码校验用例（四类危险字符、CR、TAB、尾随 CR，以及自动生成密码 20 次必须通过），并加入「校验必须单点定义且两处共用」的回归锁，同时覆盖 ss/anytls/proxy 三个脚本。
