...
show_header() {
    ...
    echo -e "  ${SKYBLUE}${BOLD}│${PLAIN} ${WHITE}${BOLD}Sing-box Multi-Protocol Tools${PLAIN} ${GREEN}${BOLD}v2.0.22${PLAIN} ${DIM}VLESS · AnyTLS · HY2 · SS${PLAIN}"
    echo -e "  ${SKYBLUE}${BOLD}╰────────────────────────────────────────────────────────╯${PLAIN}"
...
backup_config() {
    ...
    printf '%s\n' "script_version=v2.0.22" > "${BACKUP_DIR}/latest-version.txt"
