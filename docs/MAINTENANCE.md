...
- AnyTLS/VLESS/HTTP/SOCKS 网卡绑定刷新独立执行备份、配置校验、运行中服务重启及健康检查，不依赖核心版本是否发生变化。
- 刷新失败会恢复 JSON 与元数据；回滚失败保留 `.bind.*` 备份并报告路径，需人工核对后恢复。升级不得吞掉刷新失败。
- EUserv DNS 恢复失败时保留 `/etc/resolv.conf.hy2bak` 或 `.hy2absent` 标记及待恢复状态。遗留恢复文件不会被下一次 NAT64 切换覆盖；应先核对备份并恢复原 DNS。
