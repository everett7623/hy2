...
ensure_outbound_bind() {
    ...
    if [ -n "$BIND_INTERFACE" ] && ! grep -qF "\"bind_interface\": \"${BIND_INTERFACE}\"" "$ANYTLS_CONFIG" 2>/dev/null; then
        _need_rewrite=1
    fi
    ...
    if (
        _config_backup=$(mktemp "${ANYTLS_CONFIG}.bind.XXXXXX") || exit 1
        _meta_backup=$(mktemp "${ANYTLS_META}/config.env.bind.XXXXXX") || {
            rm -f "$_config_backup"; exit 1
        }
        if ! cp -p "$ANYTLS_CONFIG" "$_config_backup" || \
            ! cp -p "$ANYTLS_META/config.env" "$_meta_backup"; then
        ...
        rollback_outbound_bind() {
            local _ok=1
            cp -p "$_config_backup" "$ANYTLS_CONFIG" || _ok=0
            cp -p "$_meta_backup" "$ANYTLS_META/config.env" || _ok=0
...
main_menu() {
    while true; do
        ...
        echo -e "  ${GREEN}${BOLD}AnyTLS Management Script${PLAIN} ${DIM}v2.0.40${PLAIN}"
