...
- 保持 `install.sh`、`hy2.sh`、`ss.sh`、`anytls.sh`、`vless.sh`、`proxy.sh`、`euservhy2.sh` 可独立执行。
...
- 七个脚本的文件头版本和日期
- `install.sh`、`hy2.sh`、`ss.sh`、`anytls.sh`、`vless.sh`、`proxy.sh` 的菜单显示版本
...
- HTTP/SOCKS 变更需同步 `tests/validate_proxy.sh`；保持 mixed 入站、用户凭据、`bind_interface`、wrapper/service 的测试覆盖。
